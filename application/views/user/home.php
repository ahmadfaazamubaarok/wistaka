			...
			<a href="<?= site_url('welcome/wisata/').$key->nama_wisata ?>">
				<div class="item">
					...
					<div class="card-content">
						...
						<?php echo (strlen($key->deskripsi_wisata) > 50) ? substr($key->deskripsi_wisata, 0, 20) . "..." : $key->deskripsi_wisata; ?>
					</div>
				</div>
			</a>
			<?php endforeach ?>
...
<script>
	function searchHref() {
		...
		for (let link of links) {
			...
			if (href.toLowerCase().includes(input) || text.toLowerCase().includes(input)) {
				window.location.href = href;
				return;
			}
		}
            alert("Tidak ditemukan!"); // Menampilkan pesan jika tidak ada hasil
        }
    </script>
